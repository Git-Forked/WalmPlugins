import "WalmPlugins.PimpYourText.DiaryEntryNode";
...
function GenericTooltip:Constructor(parent, title, description)
	...
	if(parent~=nil) then
		...
		parent.MouseEnter = function (a, e)
			...
			if( x + 20 +self:GetWidth() > Turbine.UI.Display.GetWidth()) then x=Turbine.UI.Display.GetMouseX()-self:GetWidth()-10; end
			if( y + 5 + self:GetHeight() > Turbine.UI.Display.GetHeight()) then y=Turbine.UI.Display.GetMouseY()-self:GetHeight()-10; end
	...
	if(title~=nil) then
		self:AddLayer(title, Turbine.UI.Color(197/255,162/255,57/255), nil, Turbine.UI.Lotro.Font.Verdana18, Turbine.UI.FontStyle.Outline, Turbine.UI.Color( 0.3, 0.3, 0.3 ));
	end
...
function GenericTooltip:AddPictureHeader(picture, text, color, layout)
	...
	icone:SetSize(32, 32);
	icone:SetBackground(picture or "WalmPlugins/PimpYourText/pictures/tooltips/default.jpg");
...
function GenericTooltip:BuildBackground()
	...
	backUp:SetBackground("WalmPlugins/PimpYourText/pictures/tooltips/tooltip_top.tga");
	...
	self.backMid:SetBackground("WalmPlugins/PimpYourText/pictures/tooltips/tooltip_mid.tga");
	...
	self.backBottom:SetBackground("WalmPlugins/PimpYourText/pictures/tooltips/tooltip_bottom.tga");
