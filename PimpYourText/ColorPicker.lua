...
function ColorPicker:Constructor()
	...
	self:SetBackground( "WalmPlugins/PimpYourText/pictures/ColorSelectorPane.tga" );
	...
	self.closeBtn:SetBackground( "WalmPlugins/PimpYourText/pictures/close.jpg" );
	self.closeBtn:SetSize(16, 16);
	self.closeBtn:SetPosition(153, 5);
	self.closeBtn.tooltip = GenericTooltip(self.closeBtn);
	self.closeBtn.tooltip:AddLayer(_("CLOSECOLOR_BTN"), Turbine.UI.Color(1,1,1), Turbine.UI.ContentAlignment.TopLeft, Turbine.UI.Lotro.Font.Verdana14, nil);	
	...
	self.removeBtn:SetBackground( "WalmPlugins/PimpYourText/pictures/delete.jpg" );
	self.removeBtn:SetSize(16, 16);
	self.removeBtn:SetPosition(153, 23);
	self.removeBtn.tooltip = GenericTooltip(self.removeBtn);
	self.removeBtn.tooltip:AddLayer(_("REMOVECOLOR_BTN"), Turbine.UI.Color(1,1,1), Turbine.UI.ContentAlignment.TopLeft, Turbine.UI.Lotro.Font.Verdana14, nil);		
...
function ColorPicker:BuildColorsPanes()
	local colors = {"FFFFFF", "FFFF00", "FF8300", "FF0000", "6D071A", "582900", "B53389","FF5CCD", "00FF00", "095228", "B0F2B6", "00FFFF", "0000FF", "38E5DF", "D4AF37", "5A5A5A", "9E9E9E", "000000"};
	
	for i, j in pairs(colors) do
		local p = ColorPreview(j);
		p:SetParent( self );
		p:SetPosition(7+math.mod(i-1,6)*24, 7+math.floor((i-1)/6)*24);
