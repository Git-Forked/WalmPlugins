import "WalmPlugins.PimpYourText.LerpValue";

QuickLauncher = class( Turbine.UI.Window );
...
function QuickLauncher:Constructor(dialog)
	...
	self.background:SetBackground( "WalmPlugins/PimpYourText/pictures/background2.tga" );
	...
	layer1:SetBackground( "WalmPlugins/PimpYourText/pictures/logo_layer.tga" );
...
function QuickLauncher:Update( args )
	...
	--Turbine.Shell.WriteLine("<rgb=#FF5CCD>UPDATE  - delta=".. delta .." - value="..  math.floor(self.blinker:GetValue()) .."/"..  math.floor(self.blinker:GetTargetValue()) .."</rgb>");
	if (not self.blinker:NeedsUpdate()) then
		...
		--Turbine.Shell.WriteLine("<rgb=#FF5CCD>UPDATE : backpos="..backpos.. "   dep="..dep.."</rgb>");
		if(dep<0) then
