import "WalmPlugins.PimpYourText.ColorPreview";
import "WalmPlugins.PimpYourText.ColorPicker";
...
function ColorQueue:Constructor(width, height, updateFct)
	...
	self.addIcone:SetBackground( "WalmPlugins/PimpYourText/pictures/closeAdd.tga" );
	self.addIcone.tooltip = GenericTooltip(self.addIcone);
	self.addIcone.tooltip:AddLayer(_("ADDICO_TOOLTIP"), Turbine.UI.Color(1,1,1), Turbine.UI.ContentAlignment.TopLeft, Turbine.UI.Lotro.Font.Verdana14, nil);	
...
function ColorQueue:SetPreset(rgbList, preset)
	if(preset==2 or preset==3 or preset==4) then
		self.maxColors = 6;
		self.minColors = 2;
		if(rgbList==nil or table.getn(rgbList)<self.minColors or table.getn(rgbList)>self.maxColors) then rgbList={"0000FF", "FFFFFF"}; end
	else
