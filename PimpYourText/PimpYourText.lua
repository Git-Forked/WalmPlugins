--import "WalmPlugins.PimpYourText.ComboBox";
import "WalmPlugins.PimpYourText.DropDown";
import "WalmPlugins.PimpYourText.UTF";
import "WalmPlugins.PimpYourText.GenericTooltip";
import "WalmPlugins.PimpYourText.ColorQueue";


PimpYourTextDialog = class( Turbine.UI.Lotro.Window );

function PimpYourTextDialog:Constructor()
	...
	self.Closed = function(sender, args)
		if(settings.autoShowLaunchingMessage~=false) then 
			Turbine.Shell.WriteLine(_("HIDE"));
		end
		self:Stop();
	end
...
function PimpYourTextDialog:CreateComponents()
	-- Custom text to send
	...
	self.resizer:SetBackground("WalmPlugins/PimpYourText/pictures/panel_resize_icon.tga");
...
function PimpYourTextDialog:CreateColorComponents()
	self.colorTypeList = {_("COLOR_T1"), _("COLOR_T2"), _("COLOR_T3"), _("COLOR_T4")};
			...
			if(self.colorTypeList[i]==self.colorTypeDropDown:GetText()) then
				...
				if(i==2) then
					...
					elseif(settings.loadData~=nil and settings.loadData[self.tabLabel.selectedValue]~=nil and settings.loadData[self.tabLabel.selectedValue].rgbList~=nil and table.getn(settings.loadData[self.tabLabel.selectedValue].rgbList)>1) then
						...
						rgbList = settings.rainbowColors or {"FF0000", "FFFF00", "00FF00", "38E5DF", "FF5CCD", "FF00FF"}; 
					end
...
function PimpYourTextDialog:CreateDestComponents()
	...
	self.destArea.FocusLost = function ()
		...
		if(string.len(self.destArea:GetText())==0) then
			Turbine.Shell.WriteLine(_("ERROR_DEST"));
		else
			self:UpdateValidationButton();
		end
	end
	...
	self.canalList={_("CHANNEL_TALK"),_("CHANNEL_TRADE"), _("CHANNEL_COMM"), _("CHANNEL_CONF"), _("CHANNEL_RAID"), _("CHANNEL_WORLD"), _("CHANNEL_MI"), _("CHANNEL_OFF"), _("CHANNEL_MDJ"), _("CHANNEL_RDC"), _("CHANNEL_CUST1"), _("CHANNEL_CUST2"), _("CHANNEL_CUST3"), _("CHANNEL_CUST4"), _("CHANNEL_CUST5"), _("CHANNEL_CUST6"), _("CHANNEL_CUST7"), _("CHANNEL_CUST8")};
	self.commandList={_("CHANNEL_TALK_C"),_("CHANNEL_TRADE_C"), _("CHANNEL_COMM_C"), _("CHANNEL_CONF_C"), _("CHANNEL_RAID_C"), _("CHANNEL_WORLD_C"), _("CHANNEL_MI_C"), _("CHANNEL_OFF_C"), _("CHANNEL_MDJ_C"), _("CHANNEL_RDC_C"), _("CHANNEL_CUST1_C"), _("CHANNEL_CUST2_C"), _("CHANNEL_CUST3_C"), _("CHANNEL_CUST4_C"), _("CHANNEL_CUST5_C"), _("CHANNEL_CUST6_C"), _("CHANNEL_CUST7_C"), _("CHANNEL_CUST8_C")};
	self.commandid = 1;
...
function PimpYourTextDialog:CreateSwitchComponents()
	...
	self.upBtn:SetBackground( "WalmPlugins/PimpYourText/pictures/scrollbar_10_downarrow.tga" );
	self.upBtn.tooltip = GenericTooltip(self.upBtn);
	self.upBtn.tooltip:AddLayer(_("TAB_UP"), Turbine.UI.Color(1,1,1), Turbine.UI.ContentAlignment.TopLeft, Turbine.UI.Lotro.Font.Verdana14, nil);	
	...
	self.downBtn:SetBackground( "WalmPlugins/PimpYourText/pictures/scrollbar_10_uparrow.tga" );
	self.downBtn.tooltip = GenericTooltip(self.downBtn);
	self.downBtn.tooltip:AddLayer(_("TAB_DOWN"), Turbine.UI.Color(1,1,1), Turbine.UI.ContentAlignment.TopLeft, Turbine.UI.Lotro.Font.Verdana14, nil);		
...
function PimpYourTextDialog:CreateToolscomponents()
	self.resetBtn = Turbine.UI.Label();
	self.resetBtn:SetParent( self );
	self.resetBtn:SetBackground( "WalmPlugins/PimpYourText/pictures/delete.jpg" );
	self.resetBtn:SetSize(16, 16);
	self.resetBtn:SetBlendMode( Turbine.UI.BlendMode.AlphaBlend );
	self.resetBtn.tooltip = GenericTooltip(self.resetBtn, _("RESET"), _("RESET_BTN"));
	...
	self.saveBtn:SetBackground( "WalmPlugins/PimpYourText/pictures/plume.jpg" );
	self.saveBtn:SetSize(16, 16);
	self.saveBtn.tooltip = GenericTooltip(self.saveBtn, _("SAVE"), _("SAVE_BTN"));	
	...
	self.addColorTagBtn.tooltip = GenericTooltip(self.addColorTagBtn, _("CTAG1_BTN"), _("CTAG2_BTN"));
	self.addColorTagBtn:SetBackground( "WalmPlugins/PimpYourText/pictures/tagCOpen.jpg" );
	...
	self.addDefaultTagBtn.tooltip = GenericTooltip(self.addDefaultTagBtn, _("CFTAG1_BTN"), _("CFTAG2_BTN"));	
	self.addDefaultTagBtn:SetBackground( "WalmPlugins/PimpYourText/pictures/tagCClose.jpg" );
	...
	self.addUnderTagBtn.tooltip = GenericTooltip(self.addUnderTagBtn, _("UTAG1_BTN"), _("UTAG2_BTN"));	
	self.addUnderTagBtn:SetBackground( "WalmPlugins/PimpYourText/pictures/tagUOpen.jpg" );
	...
	self.addUnderCTagBtn.tooltip = GenericTooltip(self.addUnderCTagBtn, _("UFTAG1_BTN"), _("UFTAG2_BTN"));	
	self.addUnderCTagBtn:SetBackground( "WalmPlugins/PimpYourText/pictures/tagUClose.jpg" );
	...
	self.removeTagBtn.tooltip = GenericTooltip(self.removeTagBtn, _("RTAG1_BTN"), _("RTAG2_BTN"));	
	self.removeTagBtn:SetBackground( "WalmPlugins/PimpYourText/pictures/tag.jpg" );
	...
	self.configBtn:SetBackground( "WalmPlugins/PimpYourText/pictures/config.jpg" );
	self.configBtn:SetSize(16, 16);
	self.configBtn.tooltip = GenericTooltip(self.configBtn, "Configuration", _("CONFIG_BTN"));	
...
function PimpYourTextDialog:CreateGeneratorComponents()
	...
	self.qsValid:SetShortcut(Turbine.UI.Lotro.Shortcut( Turbine.UI.Lotro.ShortcutType.Alias, "/raid <rgb=#FF5CCD>TEMP MSG :P</rgb>"));	-- rose = FF5CCD
	...
	c:SetBackground( "WalmPlugins/PimpYourText/pictures/role_button_normal.tga");
	c:SetMouseVisible(false);
	c:SetZOrder(10001);
	c:SetText(_("SENDMESSAGE"));	
end

-- Update the sending button
function PimpYourTextDialog:UpdateValidationButton()
	local text = FromUTF8(self.textArea:GetText());
	
	if(self.colorTypeId==2) then
		text = string.gsub(text, "<.->", "");
		local rainbowList=self.colorQueue:GetQueueList() or {"FF0000", "FFFF00", "00FF00", "38E5DF", "FF5CCD", "FF00FF"};
		local str = "";
	...
	self.qsValid:SetShortcut(Turbine.UI.Lotro.Shortcut( Turbine.UI.Lotro.ShortcutType.Alias, finalcommand));
end

function PimpYourTextDialog:ToRGB(r, g, b)
	local str = "";
	local a1; local a2;
	a1 = math.floor(r/16);
	a2 = math.fmod(r, 16);
...
function PimpYourTextDialog:SwitchTab(delta, firstload)
	...
	if loadDefaultValues == true then
		self.textArea:SetText(_("DEFAULT_TXT"));
		self.destArea:SetText("");
		self.commandid = 1;
		self.commandDropDown:SetLabel(self.canalList[self.commandid]);
		self.command = self.commandList[self.commandid];
...
function PimpYourTextDialog:GetName()
	return "PimpYourText";
end

function PimpYourTextDialog:IsRunning()
	return ( self ~= nil );
end

function PimpYourTextDialog:Start()
	if(self~=nil) then 
		self:SetVisible(true); 
	end
end

function PimpYourTextDialog:Stop()
	self = nil;
end

function PimpYourTextDialog:UpdateResponsive()
	...
	self.qsValid:SetPosition(self:GetWidth()-self.qsValid:GetWidth()-40, self:GetHeight()-self.qsValid:GetHeight()-16);
	self.qsLbl:SetPosition(self.qsValid:GetLeft(), self.qsValid:GetTop());
	
	self.saveBtn:SetPosition( self:GetWidth()-23, self.textArea:GetTop()+self.textArea:GetHeight()/2-self.saveBtn:GetHeight()/2);	
	...
	self.removeTagBtn:SetPosition(self.colorTypeDropDown:GetLeft()+self.colorTypeDropDown:GetWidth()-20, self.colorQueue:GetTop());
